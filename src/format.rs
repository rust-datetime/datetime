...
impl<'a> Field<'a> {
    fn format<T>(&self, when: &T, w: &mut Vec<u8>, locale: &locale::Time) -> io::Result<()> where T: DatePiece+TimePiece {
        match *self {
            ...
            Field::MonthName(true, a)     => a.format(w, &locale.long_month_name(when.month() as usize)[..]),
            Field::MonthName(false, a)    => a.format(w, &locale.short_month_name(when.month() as usize)[..]),
